...
* 현재 사용 중인 Workbench 버전은 <mark style="color:red;">22.2.x 이상</mark>입니다.\
  <mark style="color:red;">→ 만약 22.2.x보다 이전 버전을 사용 중이라면, 먼저 22.2.x로 업데이트한 후 이 지침을 따르세요.</mark>
...
### 22.2로 업데이트하기 위한 선택적 준비 <a href="#optional-preparation-for-updating-to-222" id="optional-preparation-for-updating-to-222"></a>

대용량 데이터베이스가 설치된 Workbench를 22.2 또는 23.1 이전 버전에서 최신 버전으로 업데이트하는 데는 특정 데이터베이스 변경으로 인해 시간이 매우 오래 걸릴 수 있습니다. 업데이트 중에는 웹앱을 사용할 수 없습니다.

데이터베이스에 연결하고 다음 명령문을 실행하면 업데이트를 준비할 수 있습니다.

```
ALTER TABLE "file_client_results" MODIFY "file_license" mediumtext NULL;
```

이 작업이 완료되면(시간이 꽤 오래 걸리는 부분) 실제 업데이트는 평소보다 더 오래 걸리지 않을 것입니다.

#### PHP 전제 조건 <a href="#php-prerequisites" id="php-prerequisites"></a>

FossID 서버가 Debian 11, Ubuntu 20.04 또는 Ubuntu 22.04, Redhat 8 또는 RHEL Server 8에서 실행되는 경우 먼저 8.2 이상의 PHP 버전이 있는 저장소를 추가해야 합니다.
...
Debian과 Ubuntu 두 가지 모두에서 `apt remove`제거해야 할 php 버전을 반영하도록 명령을 수정합니다.

> **참고:** 일부 Debian 또는 Ubuntu 시스템에서는 패키지 관리자의 작동 방식으로 인해 사용 중인 PHP 버전에 맞는 일부 PHP 패키지가 설치되지 않을 수 있습니다. [문제 해결 페이지를](https://fossid.osbc.co.kr/help/en/installation/webapp_troubleshooting.html) 확인하세요 .
...
원하는 PHP 버전을 설정합니다(최소 요구 버전은 8.2입니다):
