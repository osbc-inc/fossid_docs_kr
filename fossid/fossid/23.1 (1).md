...
* DATABASE 업데이트
  *   버전 22.2부터 중복 데이터를 방지하기 위해 데이터베이스 수준에서 새로운 고유 제약 조건(unique constraints)이 추가되었습니다.

      기존에 중복 데이터가 있을 수 있으니 이를 확인하고 수정하는 데 도움이 되도록, 다음 명령어를 `/fossid/webapp` 디렉터리에서 실행해야 합니다:\
