...
* 이번 업그레이드의 대상 버전은 <mark style="color:red;background-color:red;">**2025.1.2**</mark>입니다.
...
* ORT는 최근 API 변경을 거쳤으며, 이 버전의 FossID Workbench를 사용하려면 최신 버전으로 업데이트해야 합니다. 배포 포털의 설치 패키지를 사용하여 이전 버전의 ORT를 설치한 경우, 먼저 이전 버전을 제거하고 /fossid/lib/ort폴더를 삭제해야 합니다. 그런 다음 배포 포털에서 새 ORT 버전(22.2)을 다운로드하여 설치하십시오.\
  \
...
설정을 확인하고 /fossid/setup/templates/nginx.conf 파일의 예시와 비교하여동기화하세요.
...
<mark style="color:red;">**PHP를 업그레이드한 경우 다음과 같이 변경하세요.**</mark>
