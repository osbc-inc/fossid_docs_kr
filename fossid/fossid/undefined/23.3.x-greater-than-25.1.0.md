...
*   <mark style="color:red;">FossID 23.3.0 버전</mark>부터 `/fossid/logs` 디렉토리 내 로그 파일을 \*\*자동으로 순환(로테이션)\*\*할 수 있는 설정 파일이 추가되었습니다.

    로그 순환 설정은 `/etc/logrotate.d/fossid-workbench` 파일을 수정하여 사용자의 필요에 맞게 조정할 수 있습니다.
*   <mark style="color:red;">FossID 23.3.0 버전</mark>에서는 새로운 nginx location 및 rewrite 설정이 추가되었습니다.\
...
* <mark style="color:red;">FossID 23.3.0 버전</mark>부터, 이전에 Microsoft OAuth2 로그인을 사용하고 있었다면 `fossid.conf` 파일에서 몇 가지 변경 사항을 적용해야 합니다. 이는 여러 인증 제공자(Identity Provider)를 지원하기 위해 일부 파라미터의 이름이 변경되었거나 새로 추가되었기 때문입니다:
