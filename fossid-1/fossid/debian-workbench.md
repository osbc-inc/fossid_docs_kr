...
[관리자 비밀번호 구성](https://www.auditoss.kr/help/en/installation/webapp_debian.html#configure-admin-password) 단계에서 생성한 사용자 이름 `fossid`과 비밀번호 로 로그인합니다 .
...
먼저, 유료 저장소를 추가해야 할 수도 있습니다 `/etc/apt/sources.list`. 아래 스니펫을 참조하세요. 설정에 따라 호스트 이름과 배포판 이름이 다를 수 있습니다.
